...
void get_time_string_full(char *buf, size_t len) {
    ...
    struct tm *t = localtime(&now);
    if (t->tm_year < 120) {
        snprintf(buf, len, "Time Not Synced");
        return;
    }
...
void add_log(const char *msg) {
    ...
    if (logCount < MAX_LOGS) {
        ...
        for (int i = 0; i < MAX_LOGS - 1; i++) {
            strcpy(eventLogs[i], eventLogs[i + 1]);
        }
...
void reset_stats(void) {
    ...
    peak_p = 0.0;
    total_Wh = 0.0;
    strcpy(peak_v_time, "N/A");
    strcpy(peak_c_time, "N/A");
    relay1_total_on_ms = 0;
    relay1_last_activation_ms = relay1_state ? get_millis() : 0;
    relay2_total_on_ms = 0;
    relay2_last_activation_ms = relay2_state ? get_millis() : 0;
}
...
void check_daily_reset(void) {
    ...
    if (t->tm_year > 120) {
        if (t->tm_hour == 6 && t->tm_min == 30 && !daily_reset_done) {
            reset_stats();
            add_log("Daily 6:30AM Reset");
            daily_reset_done = 1;
        } else if (t->tm_hour != 6 || t->tm_min != 30) {
            daily_reset_done = 0;
        }
    }
}
...
void handle_http_client(int client_fd) {
    ...
    if (strstr(buffer, "GET /api/data") != NULL) {
        ...
        char uptime2[16];
        char json[2048];
        get_time_string_full(time_str, sizeof(time_str));
        ...
        if (r == 1) {
            update_relay_timing(1, s, relay1_state);
            relay1_state = s;
            gpio_write(RELAY1_GPIO, s);
            char msg[32];
            snprintf(msg, sizeof(msg), "Manual R1 -> %s", s ? "ON" : "OFF");
            add_log(msg);
        } else if (r == 2) {
            update_relay_timing(2, s, relay2_state);
            relay2_state = s;
            gpio_write(RELAY2_GPIO, s);
            char msg[32];
            snprintf(msg, sizeof(msg), "Manual R2 -> %s", s ? "ON" : "OFF");
            add_log(msg);
        }
        save_settings();
...
int main(void) {
    ...
    gpio_export(RELAY1_GPIO);
    gpio_export(RELAY2_GPIO);
    ...
    if (i2c_fd < 0 || ioctl(i2c_fd, I2C_SLAVE, INA226_ADDRESS) < 0) {
        add_log("INA226 connection failed");
        return 1;
    }
    ...
    while (1) {
        ...
        if (now_ms - last_read >= current_interval) {
            ...
            if (!s1 || !s2 || !s3) {
                ...
                v_now = v;
                c_now = (float)raw_curr;
                p_now = p;
                if (v > peak_v) {
                    peak_v = v;
                    get_time_string_short(peak_v_time, sizeof(peak_v_time));
                }
                if (c_now > peak_c) {
                    peak_c = c_now;
                    get_time_string_short(peak_c_time, sizeof(peak_c_time));
                }
                if (p > peak_p) {
                    peak_p = p;
                }
                ...
                if (v_filtered < 0.0) {
                    ...
                    c_filtered = (c_filtered * 0.8) + (c_now * 0.2);
                }
                ...
                if (v_filtered <= v_low_1) {
                    desired_1 = 1;
                } else if (v_filtered >= v_high_1 && c_filtered >= c_high_1) {
                    desired_1 = 0;
                }

                if (desired_1 != relay1_state) {
                    if (debounce1_timer_start == 0) {
                        debounce1_timer_start = get_millis();
                    }
                    if (get_millis() - debounce1_timer_start >= debounce1_delay_ms) {
                        update_relay_timing(1, desired_1, relay1_state);
                        relay1_state = desired_1;
                        gpio_write(RELAY1_GPIO, desired_1);
                        debounce1_timer_start = 0;
                        char msg[32];
                        snprintf(msg, sizeof(msg), "Relay1 -> %s", desired_1 ? "ON" : "OFF");
                        add_log(msg);
                        save_settings();
                    }
                } else {
                ...
                if (v_filtered <= v_low_2) {
                    desired_2 = 0;
                } else if (v_filtered >= v_high_2) {
                    desired_2 = 1;
                }

                if (desired_2 != relay2_state) {
                    if (debounce2_timer_start == 0) {
                        debounce2_timer_start = get_millis();
                    }
                    if (get_millis() - debounce2_timer_start >= debounce2_delay_ms) {
                        update_relay_timing(2, desired_2, relay2_state);
                        relay2_state = desired_2;
                        gpio_write(RELAY2_GPIO, desired_2);
                        debounce2_timer_start = 0;
                        char msg[32];
                        snprintf(msg, sizeof(msg), "Relay2 -> %s", desired_2 ? "ON" : "OFF");
                        add_log(msg);
                        save_settings();
                    }
                } else {
